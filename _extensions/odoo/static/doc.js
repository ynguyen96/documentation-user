(function ($) {
    ...
    $(document).ready(function () {
        ...
        // -- Cards animation
        var set_card_color = function () {
            $main.find(".card").each(function () {
                var $card = $(this);
                var img = document.createElement('img');
                img.setAttribute('src', $card.data('img-url'));

                img.addEventListener('load', function () {
                    var vibrant = new Vibrant(img);
                    var swatches = vibrant.swatches();
                    $card.find('figcaption').css('background-color', 'rgba(' + swatches['DarkVibrant'].getRgb() + ',0.6)');
                    img.remove();
                });
            });
        };
